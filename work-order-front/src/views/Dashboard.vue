    ...
    <div class="page-header dashboard-header mb-4">
      <div class="dashboard-intro">
        ...
        <p class="text-muted small mb-0">
          今天是 {{ todayFormatted }}，以下是今日工單狀況與 Google 行事曆總覽。
        </p>
      </div>
      ...
      <button
        v-else
        class="btn btn-success shadow-sm fw-bold calendar-connect-btn"
        disabled
      >
        <i class="bi bi-check-circle-fill me-2"></i> ✅ 已成功同步 Google 日曆
        ({{ googleEvents.length }} 筆行程)
      </button>
    </div>
      ...
      <div class="col-md-3 d-flex">
        <div class="card card-pad shadow-sm border-0 bg-white h-100 w-100">
          ...
          <div class="h2 fw-bold text-warning mb-0">
            <span
              v-if="kpiStats.loading"
              class="spinner-border spinner-border-sm text-warning me-1"
            ></span>
      ...
      <div class="col-md-3 d-flex">
        <div class="card card-pad shadow-sm border-0 bg-white h-100 w-100">
          ...
          <div class="h2 fw-bold text-success mb-0">
            <span
              v-if="kpiStats.loading"
              class="spinner-border spinner-border-sm text-success me-1"
            ></span>
...
const loadKpiStats = async () => {
  try {
    ...
    realWorkOrderEvents.value = userFilteredTickets.map((t) => {
      const rawTime =
        t.createdTime || t.created_time || t.dueTime || t.due_time;
...
const fetchGoogleCalendarEvents = async (accessToken) => {
  try {
    ...
    Swal.fire(
      "同步完成",
      `已成功為您載入近期 ${googleEvents.value.length} 筆 Google 日曆私人行程！`,
      "success",
    );
  } catch (error) {
