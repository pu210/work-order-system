...
public class AdminCheckService {
    ...
    public AdminCheckService(WorkOrderRepository workOrderRepository,WorkOrderStateMachineService workOrderStateMachineService) {
        this.workOrderRepository = workOrderRepository;
        this.workOrderStateMachineService=workOrderStateMachineService;
    }

    @Transactional
    public void adminCheckAccept(ChangeStatusRequest request, Integer workOrderId) {
        ...
        if (workOrder.getStatus() != WorkOrderState.PENDING_ADMIN_ACCEPTANCE) {
            throw new IllegalStateException("目前不是管理員審核狀態");
        }
    ...
    @Transactional
    public void adminCheckReject(ChangeStatusRequest request, Integer workOrderId) {
        WorkOrder workOrder = findByWorkOrderId(workOrderId);
