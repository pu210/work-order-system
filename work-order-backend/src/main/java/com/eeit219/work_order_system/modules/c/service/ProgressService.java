...
public class ProgressService {
    ...
    @Transactional
    public void progressAccept(AcceptWorkOrderRequest request, Integer workOrderId) {
    ...
    @Transactional
    public void progressReject(RejectWorkOrderRequest request, Integer workOrderId) {
        WorkOrder workOrder = workOrderRepository.findById(workOrderId)
                .orElseThrow(() -> new ResourceNotFoundException("找不到工單"));
        if (workOrder.getStatus() != WorkOrderState.IN_PROGRESS) {
            throw new InvalidWorkOrderStateException("目前不是進行中狀態");
